...
import $ from 'jquery';
import { MATE, MAXDEPTH, INFINITE, NOMOVE, BOOL, GameBoard, PVENTRIES, FROMSQ, TOSQ, MFLAGCAP, BRD_SQ_NUM, PCEINDEX, Kings } from './defs';
...
export function IsRepetition() {
  ...
  for (index = GameBoard.hisPly - GameBoard.fiftyMove; index < GameBoard.hisPly - 1; index += 1) {
...
export function AlphaBeta(alpha, beta, depth) {
  ...
  for (
    ...
    Legal += 1;
    Score = -AlphaBeta(-beta, -tAlpha, tDepth - 1);
...
export function UpdateDOMStats(domScore, domDepth) {
  ...
  return {
    OrderingOut: `Ordering: ${((SearchController.fhf / SearchController.fh) * 100).toFixed(2)}%`,
    DepthOut: `Depth: ${domDepth}`,
    ScoreOut: scoreText,
    NodesOut: `Nodes: ${SearchController.nodes}`,
    TimeOut: `Time: ${(($.now() - SearchController.start) / 1000).toFixed(1)}s`,
    BestOut: `BestMove: ${PrMove(SearchController.best)}`,
  };
}
